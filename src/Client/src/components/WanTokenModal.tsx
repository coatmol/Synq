  ...
  return (
    <Modal isOpen={isOpen} onOpenChange={(open) => !open && onClose()}>
      <Modal.Backdrop className="bg-black/60 backdrop-blur-sm">
        <Modal.Container>
          <Modal.Dialog
            className="bg-zinc-900 border border-zinc-800 shadow-2xl rounded-xl overflow-hidden min-w-[400px]">
              ...
              {!natInfo ? (
                ...
                <div className="flex flex-col gap-5">
                  <div className="bg-zinc-950 p-4 rounded-lg text-sm border border-zinc-800">
                    ...
                    <p className="text-zinc-400 mb-1">NAT Type: <span
                      className="text-zinc-200">{natInfo.natType === "Symmetric" ? "⚠️ Symmetric" : `✅ ${natInfo.natType}`}</span>
                    </p>
                    <p className="text-zinc-400 mb-1">STUN Servers: <span
                      className="text-zinc-200">{natInfo.servers?.filter((s: any) => s.reachable).length || 0}/{natInfo.servers?.length || 0} reachable</span>
                    </p>
                    <p className="text-zinc-400">Hole-punching: <span
                      className="text-zinc-200">{natInfo.canHolePunch ? "Supported" : "NOT supported"}</span></p>
                    {natInfo.natType === "Symmetric" && (
                      <p className="text-amber-500 text-xs mt-2 bg-amber-500/10 p-2 rounded">You may need
                  ...
                  {step === "preflight" && (
                    <div className="flex flex-col gap-4">
                      <p className="text-sm text-zinc-400">To connect, either generate an invite token, or paste an
                        invite you received.</p>
                      <Button className="bg-emerald-600 hover:bg-emerald-500 text-white font-medium"
                              onPress={generateOffer} isDisabled={loading}>
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button onPress={() => navigator.clipboard.writeText(tokenA)}
                                  className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border-none shrink-0 px-4">Copy</Button>
                        </div>
                      </div>
            ...
            <Modal.Footer className="border-t border-zinc-800 p-4 bg-zinc-900/50 flex justify-end gap-3">
              <Button variant="secondary" className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border-none"
                      onPress={onClose}>
                {step === "connected" ? "Close" : "Cancel"}
              </Button>
              {step === "wait" && (
                <Button
                  className="bg-emerald-600 hover:bg-emerald-500 text-white font-medium shadow-[0_0_15px_rgba(16,185,129,0.3)] px-6"
                  isDisabled={!tokenB || loading} onPress={handleConnect}>
