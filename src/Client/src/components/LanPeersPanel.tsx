    ...
    <div className="flex items-center gap-2 pr-2">
          ...
          <Tooltip key={peer.id} delay={0} closeDelay={0}>
            ...
            <Tooltip.Content placement="bottom" className="bg-zinc-900 border border-zinc-800 text-zinc-200">
              <div className="flex flex-col gap-0.5 p-1">
                ...
                <span
                  className="text-[10px] text-zinc-500 font-mono">{peer.ip ? `${peer.ip}:${peer.port}` : 'Local Client'}</span>
              </div>
            </Tooltip.Content>
          </Tooltip>
        ))}
      ...
      <div className="flex items-center border-zinc-800/50 pl-2 gap-1">
        <Tooltip delay={0} closeDelay={0}>
          <Tooltip.Trigger>
            <Button isIconOnly size="sm" variant="secondary" onPress={() => setIsWanModalOpen(true)}
                    className="rounded-full w-8 h-8 min-w-8 bg-zinc-800/80 hover:bg-zinc-700 text-zinc-300 border border-zinc-700/50 transition-all"
                    aria-label="Add WAN Peer">
              <PlusCircle className="w-4 h-4"/>
            </Button>
          </Tooltip.Trigger>
          <Tooltip.Content placement="bottom"
                           className="bg-zinc-900 border border-zinc-800 text-zinc-200 p-1 px-2 text-xs">
