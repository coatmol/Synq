...
export function WelcomeScreen({onOpenEditor}: WelcomeScreenProps) {
  ...
  const handleNewProject = async (peerOverride?: { ip: string, port: number, name: string, password?: string }) => {
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).external && (window as any).external.sendMessage) {
        ...
        if (targetPeer) {
          await api.connectManualPeer({
            ip: targetPeer.ip,
            port: targetPeer.port,
            password: targetPeer.password
          });
        }
  ...
  const handleOpenRecent = async (path: string) => {
    if (typeof window !== 'undefined' && (window as any).external && (window as any).external.sendMessage) {
      ...
      if (pendingPeer) {
        await api.connectManualPeer({
          ip: pendingPeer.ip,
          port: pendingPeer.port,
          password: pendingPeer.password
        });
      }
    }
  };
  ...
  return (
    <div
      ...
      <div
        className="w-full md:w-64 h-[40vh] md:h-full bg-[#18181b] border-t md:border-t-0 md:border-r border-zinc-800/60 flex flex-col select-none shrink-0">
        <div
          ...
          <div className="flex flex-col items-center max-w-md w-full px-6">
            ...
            <div className="flex flex-col items-center mb-10 select-none">
              <div className="w-24 h-24 mb-4 relative flex items-center justify-center">
                <img src="/Synq3.png" alt="Synq Logo"
            ...
            {/* Options List */}
            <div
              className="w-full flex flex-col bg-[#27272a] rounded-xl border border-zinc-700/80 p-1.5 shadow-2xl">
              ...
              {!showPeers && !pendingPeer ? (
                ...
                <div className="flex flex-col w-full p-4">
                  <span
                    className="text-[13px] font-medium text-zinc-200 mb-2">Select a local folder to sync with {pendingPeer.name}</span>
                  <Button
                    onPress={() => handleNewProject()}
                ...
                <div className="flex flex-col w-full p-2 min-h-[250px]">
                  <div
                    className="flex justify-between items-center px-2 py-2 border-b border-zinc-800/50 mb-2">
                    <span
                      className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Local Network Peers</span>
                    {peers.length === 0 && <Spinner size="sm" color="current"/>}
                  </div>

                  <div
                    className="flex-1 overflow-y-auto flex flex-col gap-1 custom-scrollbar max-h-48 px-1">
                    {peers.length === 0 ? (
                      <div
                        className="flex-1 flex items-center justify-center text-zinc-600 text-xs italic py-4">
                        Searching for peers...
                      </div>
                    ) : (
                      peers.map(peer => (
                        <div
                          key={peer.id}
                          onClick={() => verifyAndSetPendingPeer({
                            ip: peer.ip,
                            port: peer.port,
                            name: peer.name
                          })}
                        ...
                        >
                          ...
                          <div className="flex flex-col flex-1 min-w-0">
                                                        <span
                                                          className="text-[13px] font-medium text-zinc-200 truncate group-hover:text-emerald-400 transition-colors">{peer.name}</span>
                            <span
                              className="text-[10px] text-zinc-500 font-mono truncate">{peer.ip}:{peer.port}</span>
                          </div>
                          <ArrowRight
                            className="w-4 h-4 text-zinc-600 group-hover:text-emerald-400 transition-colors mr-2"/>
                        </div>
                      ))
                    )}
                  </div>
